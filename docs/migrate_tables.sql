-- ============================================================================
-- SCRIPT DE MIGRAÇÃO - ATUALIZAR TABELAS EXISTENTES
...
-- 6. VERIFICAR E CRIAR FUNÇÕES NECESSÁRIAS
DO $$
BEGIN
    -- Função para gerar token de recuperação de senha
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'generate_password_reset_token') THEN
        ...
        BEGIN
            -- Buscar usuário
            SELECT id INTO user_id FROM usuarios WHERE email = user_email AND ativo = true;
            ...
            VALUES (user_id, user_email, reset_token, NOW() + INTERVAL '1 hour')
            ...
            RETURN QUERY SELECT reset_token, NOW() + INTERVAL '1 hour';
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE 'Função generate_password_reset_token criada';
    END IF;
    
    -- Função para verificar token de recuperação
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'verify_password_reset_token') THEN
        CREATE OR REPLACE FUNCTION verify_password_reset_token(reset_token VARCHAR)
        RETURNS TABLE(user_id UUID, email VARCHAR, valid BOOLEAN) AS $$
        DECLARE
            found_user_id UUID;
            found_email VARCHAR;
        BEGIN
            SELECT pr.user_id, pr.email INTO found_user_id, found_email
            ...
            IF found_user_id IS NOT NULL THEN
                RETURN QUERY SELECT found_user_id, found_email, true;
            ELSE
                RETURN QUERY SELECT NULL::UUID, NULL::VARCHAR, false;
            END IF;
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE 'Função verify_password_reset_token criada';
    END IF;
    
    -- Função para limpeza de dados antigos
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'cleanup_auth_data') THEN
        ...
        BEGIN
            ...
            DELETE FROM user_login_attempts WHERE created_at < NOW() - INTERVAL '30 days';
            
            RETURN 'Limpeza concluída. Tokens expirados removidos: ' || deleted_count;
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE 'Função cleanup_auth_data criada';
    END IF;
    
    RAISE NOTICE 'Verificação das funções concluída';
END $$;
...
-- 8. VERIFICAR E CRIAR TRIGGERS NECESSÁRIOS
DO $$
BEGIN
...
-- 9. VERIFICAR E CONFIGURAR RLS (Row Level Security)
DO $$
BEGIN
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Todas as tabelas, funções e políticas foram verificadas e criadas/atualizadas.';
